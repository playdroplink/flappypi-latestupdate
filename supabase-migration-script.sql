-- Connect to your Supabase database and run this migration
...
-- Update game_mode enum to include all supported modes (drop and recreate constraint)
ALTER TABLE leaderboard 
...
-- Update leaderboard_stats to include new game modes (if the table exists)
INSERT INTO leaderboard_stats (game_mode) 
SELECT mode FROM (VALUES ('screampi'), ('dinopi'), ('challenge')) AS modes(mode)
WHERE EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'leaderboard_stats')
...
SELECT 'Migration completed successfully! Added game_data column and created unified_leaderboard_view.' as status;
