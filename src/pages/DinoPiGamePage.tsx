...
const DinoPiGamePage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (path.includes('/dino-pi/classic')) {
      setSelectedMode('classic');
    } else if (path.includes('/dino-pi/endless')) {
      setSelectedMode('endless');
    } else if (path.includes('/dino-pi/challenge')) {
      setSelectedMode('challenge');
    } else if (path.includes('/dino-pi-game')) {
      setSelectedMode('classic'); // Default to classic for /dino-pi-game
    }
  }, [location.pathname]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-orange-50 to-orange-100 flex flex-col items-center justify-center p-4">
      ...
      <div className="flex gap-2 mb-6">
        <Button
          ...
          className={`${selectedMode === 'classic' ? 'bg-green-500 text-white' : 'bg-green-100 text-green-700 hover:bg-green-200'} font-bold px-4 py-2`}
        >
 Classic
        </Button>
            <Button
          onClick={() => navigate('/dino-pi/endless')}
          className={`${selectedMode === 'endless' ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-700 hover:bg-blue-200'} font-bold px-4 py-2`}
            >
          🌍 Endless
            </Button>
            <Button
          onClick={() => navigate('/dino-pi/challenge')}
          className={`${selectedMode === 'challenge' ? 'bg-purple-500 text-white' : 'bg-purple-100 text-purple-700 hover:bg-purple-200'} font-bold px-4 py-2`}
        >
          🏆 Challenge
        </Button>
      </div>
