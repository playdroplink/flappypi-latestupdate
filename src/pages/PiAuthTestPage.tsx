      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="bg-white rounded-lg p-4 mb-6 shadow-lg">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
            ...
            <div><strong>Pi SDK Available:</strong> {debugInfo.hasPiSDK ? '✅ Yes' : '❌ No'}</div>
            <div><strong>Pi Network Subdomain:</strong> {debugInfo.isPiNetworkSubdomain ? '✅ Yes' : '❌ No'}</div>
        ...
        <div className="bg-white rounded-lg p-4 mb-6 shadow-lg">
          ...
          <div className="space-y-2">
            {debugInfo.isPiNetworkSubdomain && (
              <div className="flex items-center gap-2 text-green-600"><span>✅</span><span>Pi Network Subdomain detected: {debugInfo.hostname}</span></div>
            )}
            {debugInfo.hasPiSDK && (
              <div className="flex items-center gap-2 text-green-600"><span>✅</span><span>Pi SDK is available</span></div>
            )}
            {!debugInfo.hasPiSDK && (
              <div className="flex items-center gap-2 text-red-600"><span>❌</span><span>Pi SDK is not available</span></div>
            )}
            {debugInfo.userAgent.includes('PiBrowser') && (
              <div className="flex items-center gap-2 text-green-600"><span>✅</span><span>Pi Browser detected</span></div>
            )}
          </div>
        </div>
