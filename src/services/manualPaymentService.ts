...
// Manual Payment Transaction Interface
...
class ManualPaymentService {
  ...
  private readonly SECURITY_KEY = 'flappypi_manual_payment_2024';
  ...
  generateTransactionId(): string {
    ...
    return `FLAPPY_TXN_${timestamp}_${hash.substring(0, 8).toUpperCase()}`;
  }
  ...
  async generateQRCode(walletAddress: string, amount: number, memo: string): Promise<string> {
    try {
  ...
  async createManualTransaction(
    ...
    totalPiAmount: number,
    orderNotes?: string
  ): Promise<ManualPaymentTransaction> {
    ...
    const qrCodeDataUrl = await this.generateQRCode(
      this.MERCHANT_WALLET_ADDRESS,
      totalPiAmount,
      memo
    );

    const transaction: ManualPaymentTransaction = {
      transactionId,
      orderId,
      productId,
      productName,
  ...
  private storeTransaction(transaction: ManualPaymentTransaction): void {
    try {
      ...
      localStorage.setItem('flappypi-manual-transactions', JSON.stringify(existingTransactions));
    } catch (error) {
  ...
  getStoredTransactions(): ManualPaymentTransaction[] {
    try {
      const stored = localStorage.getItem('flappypi-manual-transactions');
  ...
  getTransaction(transactionId: string): ManualPaymentTransaction | null {
    const transactions = this.getStoredTransactions();
    return transactions.find(t => t.transactionId === transactionId) || null;
  }

  // Update transaction status
  updateTransactionStatus(transactionId: string, status: ManualPaymentTransaction['status']): boolean {
    try {
      const transactions = this.getStoredTransactions();
      ...
      if (transactionIndex !== -1) {
        ...
        localStorage.setItem('flappypi-manual-transactions', JSON.stringify(transactions));
        return true;
      }
  ...
  // Verify payment using Pi Ledger
  async verifyPayment(verification: ManualPaymentVerification): Promise<{
    success: boolean;
  ...
  isTransactionExpired(transaction: ManualPaymentTransaction): boolean {
    return new Date() > transaction.expiryTime;
  }

  // Get transaction expiry time remaining
  getTransactionTimeRemaining(transaction: ManualPaymentTransaction): number {
    const now = new Date();
  ...
  cleanupExpiredTransactions(): void {
    try {
      ...
      if (validTransactions.length !== transactions.length) {
        localStorage.setItem('flappypi-manual-transactions', JSON.stringify(validTransactions));
