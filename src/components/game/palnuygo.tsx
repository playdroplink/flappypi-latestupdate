...
const ClassicMode: React.FC<ClassicModeProps> = ({ mode = 'classic', challenge }) => {
  ...
  useEffect(() => {
    ...
    const handleCollisionWithPowerUps = () => {
      ...
      if (getActiveEffects().hasExtraLife && getPowerUpStatus('extra_life').isActive) {
    ...
    for (const pipe of pipes) {
      ...
      if (pipe.x < birdRight && pipe.x + PIPE_WIDTH > birdLeft) {
        ...
        if (!inGap) {
                      console.log(`💥 Pipe collision detected in palnuygo mode:`, {
  ...
  }, [birdY, pipes, coinPos, gameStarted, gameOver, isInvincible, profile, getActiveEffects, getPowerUpStatus]);
  ...
  const handleActivatePowerup = (id: string) => {
    const pu = footerPowerUps.find(p => p.id === id);
    if (!pu || pu.quantity === 0) return;
    // TODO: Apply the powerup effect in-game (set state, trigger animation, etc.)
    // Example: setActivePowerup(id), setPowerupTimer(id, duration), etc.
    // Deduct from inventory/profile
    if (profile && updateProfile) {
      const current = profile.owned_power_ups || {};
      if (current[id] > 0) {
        updateProfile({ owned_power_ups: { ...current, [id]: current[id] - 1 } });
        toast({ title: `${pu.name} Activated!`, description: `You used a ${pu.name} powerup.`, duration: 3000 });
      }
    }
  };
  ...
  // Add missing coinMultiplier state
