...
const AD_DIALOGS = [
  ...
  "You run, we grow — with Pi from ads! 🚀",
  "Thanks! Your ad helped fund future Pi prizes! 🎁",
  "Ads keep the prehistoric world open and rewards flowing! ☁️",
  ...
  "Thanks for helping us run higher! ✈️",
  ...
  "The Dino Pi future? You're helping fund it! 🧠"
];

const ADFREE_DIALOGS = [
  "No ads, just pure running! 🎮",
  "Enjoy a cleaner prehistoric world — go Ad-Free! 🌤️",
  "Want peace? Upgrade to Ad-Free! ☁️",
  "No ads, more focus. Run freely! ",
  "Ad-Free running? You got it! 🏃",
  "Unlock ad-free prehistoric world now! 🔓",
  "Less ads, more runs. Let's go! 💨",
  "Say goodbye to ads forever! 👋",
  "Tired of ads? Go premium! 🛒",
  "Upgrade for an ad-free life! 🌈",
  "Smooth prehistoric world await — no ads! 🚀",
  "Go Ad-Free, run non-stop! 🔄",
  "One price, endless clean running! 💰",
...
const DinoPiReviveModal: React.FC<DinoPiReviveModalProps> = ({
  ...
  useEffect(() => {
    const handleSubscriptionExpired = (event: CustomEvent) => {
      console.log('📢 Subscription expired event received in DinoPiReviveModal');
  ...
  const handleCoinRevive = async () => {
    setIsLoadingAd(false);
    if (await spendCoins(progressiveReviveCost, 'Revive in Dino Pi game')) {
      toast({
  ...
  const handleWatchAdToRevive = async () => {
    console.log('🎬 [DINO REVIVE DEBUG] Starting ad watch process...');
    setIsLoadingAd(true);
    setAdError(false);
    ...
    try {
      ...
      console.log(`🎬 [DINO REVIVE DEBUG] Requesting ad for revive in ${currentGameMode} mode`);
      ...
      console.log('🎬 [DINO REVIVE DEBUG] Ad result:', result);
      
      if (result.success && (result.shown || !result.shown)) {
        // STEP 2: Ad completed successfully, now do EXACT SAME revive as coin revive
        console.log('🎬 [DINO REVIVE DEBUG] Ad completed successfully, calling onRevive - SAME AS COIN REVIVE');
        ...
        console.log('🎬 [DINO REVIVE DEBUG] Ad failed:', result.description);
        setAdError(true);
        toast({
          title: "Ad Failed",
      ...
      console.error('🎬 [DINO REVIVE DEBUG] Ad error:', error);
      setAdError(true);
      toast({
        title: "Ad Error",
    ...
    return (
      <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[10000] p-4">
        <div className="bg-white rounded-2xl max-w-sm w-full p-8 text-center shadow-2xl flex flex-col items-center">
          {/* Dinosaur Character Display */}
          {characterImage && (
            <div className="flex justify-center mb-2">
              <img 
                src={characterImage} 
                alt="Your Dinosaur Character" 
                className="w-16 h-16 sm:w-20 sm:h-20 drop-shadow-lg animate-bounce" 
                onError={(e) => {
                  console.log('❌ Dinosaur character image failed to load in DinoPiReviveModalSubscription:', characterImage);
          ...
          <p className="mb-4 text-gray-600">No ads. Run on, Pi-oneer!</p>
  ...
  return (
    <div className={`fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[10000] p-2 sm:p-4 transition-opacity duration-400 ${visible ? 'opacity-100' : 'opacity-0'}`}>
      <div className={`bg-white rounded-xl sm:rounded-2xl max-w-md w-full shadow-2xl overflow-hidden transform transition-all duration-400 ${visible ? 'scale-100' : 'scale-90'}`}>
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-500 to-red-500 text-white p-4 sm:p-6 text-center">
          {/* Dinosaur Character Display */}
          {characterImage && (
            <div className="flex justify-center mb-2">
              <img 
                ...
                className="w-16 h-16 sm:w-20 sm:h-20 drop-shadow-lg animate-bounce" 
                onError={(e) => {
                  console.log('❌ Dinosaur character image failed to load in DinoPiReviveModal:', characterImage);
                  e.currentTarget.src = '/dino pi/dino_0.png';
                }}
              />
            </div>
        ...
        <div className="p-4 sm:p-6 space-y-3 sm:space-y-4">
          ...
          {!noCancel && (
            <div className="pt-2 border-t border-gray-200">
              <button
                onClick={() => { console.log('DinoPiReviveModal: End Game (cancel) button clicked'); onDecline(); }}
